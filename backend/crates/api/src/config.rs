...
impl AppConfig {
    pub fn from_env() -> Self {
        ...
        let url = std::env::var("MNEMO_LLM_URL").unwrap_or_else(|_| "http://tensorzero:3000".into());
        let api_key = std::env::var("MNEMO_LLM_API_KEY").unwrap_or_else(|_| "none".into());
        let model = std::env::var("MNEMO_LLM_MODEL").unwrap_or_else(|_| "qwen3:8b".into());

        let llm = LlmConfig { provider, url, api_key, model };
        info!("[LLM] Using TensorZero -> {} model={} (provider=tensorzero)", llm.url, llm.model);
        Self { llm }
    }
}
